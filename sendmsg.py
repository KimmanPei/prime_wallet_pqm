        ...
        try:
            ...
            while msg != b'':
                ...
                # 3 is ether
                    ...
                    seq_bytes = int.to_bytes(self.sequence, 4, "big")
                    tx_slice = seq_bytes+msg_slice
                    eth_txmsg = transaction.prepare_ethtx_data(tx_slice)
                    self.sequence += 1
                    print("tx_msg_slice:", tx_slice.hex())
                    print("tx_msg_slice len:", len(tx_slice))
                    try:
...
def eth_session_filter(user_dbname, eth_address, eth_block_since, session_id):
    ...
    for i in range(len(txid_list)):
        ...
        #raw_msg = transaction.eth_get_rawmsg2(txid_list[i])
...
if __name__ == '__main__':
    ...
    eth_address = "0xF735082655773Cf1ddb99bE032BB8dC93D589b31"  # must be checksum address
    ...
    ss.start(username, 8, aes_key, btc_address, ltc_address, eth_address)
    #print("IV:", ss.msg_IV.hex())

    text_byte = b'12345'*500
    #text_byte = b'hello'
    ss.stage_text(text_byte)
    is_send = ss.send_all()
